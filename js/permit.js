...
function PermitChart(permitData){
    let self = this;
    self.permitData = permitData;
    self.init();
};

PermitChart.prototype.init = function(){
    let self = this;
    self.margin = {top: 20, right: 175, bottom: 30, left: 175};
    self.svgBounds = d3.select("#permit").node().getBoundingClientRect();
    self.svgWidth = self.svgBounds.width - self.margin.left - self.margin.right;
    self.svgHeight = self.svgWidth/2;
    

    const svg = d3.select("#permit").append("svg")
        .attr("width", 450)
        .attr("height", 400)
        .attr("class", "permits");
